...
def evaluate_outputs(output_dir: str | Path) -> dict[str, Any]:
    ...
    for path in sorted((output_dir / "repairs").glob("*.repair.json")):
        ...
        original = str(payload.get("original_prediction", ""))
        repaired = str(payload.get("repaired_prediction", ""))
        original_correct = bool(answer and original == answer)
        repaired_correct = bool(answer and repaired == answer)
        applied = int(payload.get("repair_applied_count", 0))
        attempted = int(payload.get("repair_attempted_count", 0))
        rows.extend([
        ...
        for record in trace_records:
            ...
            root_rows.append({
                ...
                "localized_step": diagnosis.get("localized_step"),
                "anomaly": diagnosis.get("anomaly_score", 0.0),
                "necessity": diagnosis.get("necessity_score", 0.0),
                "sufficiency": diagnosis.get("sufficiency_score", 0.0),
                "ict": diagnosis.get("interventional_consistency", 0.0),
                "root_score": diagnosis.get("root_score", diagnosis.get("causal_score", 0.0)),
                "acceptance_lcb": acceptance.get("lcb", float("nan")),
        ...
        pair_rows.append({
            ...
            "repair_attempted": attempted > 0,
            "repair_applied": applied > 0,
            "attempted_traces": attempted,
            "applied_traces": applied,
            ...
            "mean_root_score": float(np.mean([item["diagnosis"].get("root_score", item["diagnosis"].get("causal_score", 0.0)) for item in trace_records])) if trace_records else 0.0,
            "mean_necessity": float(np.mean([item["diagnosis"].get("necessity_score", 0.0) for item in trace_records])) if trace_records else 0.0,
            "mean_sufficiency": float(np.mean([item["diagnosis"].get("sufficiency_score", 0.0) for item in trace_records])) if trace_records else 0.0,
        })
    ...
    method_frame.to_csv(table_dir / "per_example_methods.csv", index=False)
    pair_frame.to_csv(table_dir / "per_example_repair.csv", index=False)
    root_frame.to_csv(table_dir / "per_trace_root_localization.csv", index=False)
    ...
    if not method_frame.empty:
        for (task, method), subset in method_frame.groupby(["task", "method"], dropna=False):
            values = subset["correct"].astype(float).tolist()
            low, high = _bootstrap_ci(values, seed=17)
            by_task.append({"task": task, "method": method, "n": int(len(subset)), "accuracy": float(np.mean(values)), "ci95_low": low, "ci95_high": high})
    pd.DataFrame(method_summaries).to_csv(table_dir / "summary_methods.csv", index=False)
    pd.DataFrame(by_task).to_csv(table_dir / "summary_by_task.csv", index=False)
    ...
    if not pair_frame.empty:
        original_accuracy = float(pair_frame["original_correct"].mean())
        repaired_accuracy = float(pair_frame["repaired_correct"].mean())
        wrong_to_correct = int(pair_frame["wrong_to_correct"].sum())
        correct_to_wrong = int(pair_frame["correct_to_wrong"].sum())
        ...
        repair_summary.update({
            ...
            "net_accuracy_gain": repaired_accuracy - original_accuracy,
            "wrong_to_correct": wrong_to_correct,
            "correct_to_wrong": correct_to_wrong,
            "no_harm_rate": 1.0 - correct_to_wrong / originally_correct if originally_correct else 1.0,
            "repair_attempt_rate": float(pair_frame["repair_attempted"].mean()),
            "repair_apply_rate": float(pair_frame["repair_applied"].mean()),
            "mean_consistency_gain": float((pair_frame["mean_selected_consistency"] - pair_frame["mean_original_consistency"]).mean()),
            "mean_root_score": float(pair_frame["mean_root_score"].mean()),
            "mean_necessity": float(pair_frame["mean_necessity"].mean()),
            "mean_sufficiency": float(pair_frame["mean_sufficiency"].mean()),
